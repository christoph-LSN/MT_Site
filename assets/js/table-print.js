(function () {
   ...
   * The caption contains a useful domain-specific subtitle such as
  ...
  /**
   * Create or reuse a host container above the table where the print button
   * will be rendered.
   */
  ...
  /**
   * Check whether the table is already an active DataTable instance.
   */
  ...
  function attachPrintButton() {
    ...
    // Prevent duplicate attachment if the script runs more than once.
      ...
      buttons: [
        {
          ...
          // Main print title shown by DataTables in the print window.
          title: printTitle,

          // Export only the currently visible columns and the applied search/order state.
          exportOptions: {
            columns: ':visible',
            modifier: {
              search: 'applied',
          ...
          /**
           * Customize the generated print window.
           * Here we add:
           * - a CSS class on the print document body
           * - the table caption as a subheading
           * - a fixed source line
           * - additional classes on the printed table
           */
    ...
    // Append the buttons container into our custom host above the table.
